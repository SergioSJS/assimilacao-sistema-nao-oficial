...
export class InfectadoSheet extends api.HandlebarsApplicationMixin(sheets.ActorSheetV2) {
    ...
    async _onRollInstinto(event) {
        ...
        if (isAssimilada) {
            // Ordem fixa igual ao select da ficha (não usar Object.keys que é arbitrário)
            const instintosOrdem = [
            ...
            let opcoes = `<option value="">(Instinto)</option>`;
            for (let { key, label } of instintosOrdem) {
                if (actor.system.instintos[key] !== undefined)
                    opcoes += `<option value="${key}">${label}</option>`;
            }
            const instintoLabel = instintosOrdem.find(i => i.key === instintoKey)?.label ?? instintoKey;
            const contentHTML = `<form><div class="form-group"><label>Escolha o segundo Instinto para somar com ${instintoLabel}:</label><select id="segundoInstinto" name="segundoInstinto">${opcoes}</select></div><p style="font-size: 11px; color: #666;">Dois instintos rodando na margem de Mutação (1d12).</p></form>`;
                    ...
                    buttons: [{
                        ...
                        callback: async (event, button) => {
                            ...
                            await submitAssimilacaoRoll({
                                actor,
                                label: `${instintoKey.toUpperCase()} + ${inst2.toUpperCase()}`,
                                nInstinto,
                                nAptidao: nInst2,
                                tipo: "assimilada"
                            });
    ...
    async _onRollAptidao(event) {
        ...
        await submitAssimilacaoRoll({
            actor,
            label: `${instintoCombo.toUpperCase()} + ${aptidaoKey.toUpperCase()}`,
            nInstinto,
            nAptidao,
            tipo: "normal"
        });
